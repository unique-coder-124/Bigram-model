...
# hyperparameters
batch_size = int(input(BLUE + 'batch size \'powers of 2\': ' + RESET)) # how many independent sequences will we process in parallel?
block_size = int(input(BLUE + 'block size \'powers of 2\' (atleast 2 times batch size): ' + RESET)) # what is the maximum context length for predictions?
max_iters = int(input(BLUE + 'max iterations \'multiple of 100\': ' + RESET))
eval_interval = 1
learning_rate = 1e-2
...
class Block(nn.Module):
    ...
    def forward(self, x):
        x = x + checkpoint(self.sa, self.ln1(x), use_reentrant=True)
        x = x + checkpoint(self.ffwd, self.ln2(x), use_reentrant=True)
        return x
